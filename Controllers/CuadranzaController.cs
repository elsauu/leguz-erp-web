...
{
    public class CuadranzaController : Controller
    {
        private readonly LeguzContext _context;

        public CuadranzaController(LeguzContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index(string? fecha)
        {
            ...
            ViewData["Title"] = "Cuadranza Diaria";
            ...
            var registros = await _context.DailyRouteRecords
        ...
        public async Task<IActionResult> Capturar(int rutaId, string? fecha)
        {
            ...
            var existing = await _context.DailyRouteRecords
                .FirstOrDefaultAsync(r => r.Date == date && r.DeliveryRouteId == rutaId);

            if (existing != null)
                return RedirectToAction("Editar", new { id = existing.DailyRouteRecordId });

            ViewData["Title"] = $"Capturar Cuadranza — Ruta {ruta.Number:D2}";
            await CargarViewBag(ruta, date, null);

            var record = new DailyRouteRecord { Date = date, DeliveryRouteId = rutaId };
            return View(record);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Capturar(DailyRouteRecord record)
        {
            ...
            CalcularFaltantes(record);
            record.CreatedAt = DateTime.Now;
            record.CreatedBy = HttpContext.Session.GetString("UserFullName");
            _context.DailyRouteRecords.Add(record);
            await _context.SaveChangesAsync();

            TempData["Success"] = "Cuadranza capturada correctamente.";
        ...
        public async Task<IActionResult> Editar(int id)
        {
            ...
            var record = await _context.DailyRouteRecords
                .Include(r => r.DeliveryRoute)
                .FirstOrDefaultAsync(r => r.DailyRouteRecordId == id);

            if (record == null) return NotFound();

            ViewData["Title"] = $"Editar Cuadranza — Ruta {record.DeliveryRoute.Number:D2}";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Editar(int id, DailyRouteRecord record)
        {
            if (id != record.DailyRouteRecordId) return NotFound();
            ...
            {
                var ruta = await _context.DeliveryRoutes.FindAsync(record.DeliveryRouteId);
                ViewData["Title"] = $"Editar Cuadranza — Ruta {ruta?.Number:D2}";
                await CargarViewBag(ruta!, record.Date, record.SalespersonId);
                return View("Capturar", record);
            }
            ...
            TempData["Success"] = "Cuadranza actualizada correctamente.";
            return RedirectToAction("Index", new { fecha = record.Date.ToString("yyyy-MM-dd") });
        }
        ...
        private static void CalcularFaltantes(DailyRouteRecord r)
        {
            r.ShortageKilos = (r.LoadHotKilos + r.LoadHalfKilo + r.LoadTacoKilo)
                - r.KilosSold - r.KilosReturned;
            r.ShortagePackages = (r.LoadPackages + r.LoadHalfPackage + r.LoadTacoPackage)
                - r.PackagesSold - r.PackagesReturned;
        }
    }
}
